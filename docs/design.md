...
- `other_names[]` — former/DBA names (`first_name`/`last_name` or `organization_name`, `type`)
- `practiceLocations[]`, `endpoints[]` (FHIR/Direct; often empty `[]`)
...
notFound: Array<{ npi: string; reason: string }>  // valid format, no record (or API Errors[])
```

`FullProviderRecord` carries: `npi`, `type`, basic block (name/org fields, credential, sex, status, sole proprietor, enumeration + last-updated dates), **all** `taxonomies[]` (code, description, primary, license, state, taxonomy_group), **all** `addresses[]` (purpose, address_type, lines, city/state/zip, country_code, country_name, phone/fax), `identifiers[]`, `otherNames[]`, `practiceLocations[]`, `endpoints[]`. **Curate-nothing fidelity** — medical/correctness-sensitive data; pass through the full record, only renaming/normalizing field names and the `status` enum.
...
| `none_found` | `NotFound` | Every requested NPI returned no record | Verify the NPI(s); deactivated or never-enumerated numbers return nothing. Search by name to confirm. |

(Partial misses do **not** throw — they land in `notFound[]`; `none_found` fires only when *all* miss.)
...
| # | Call | Purpose | Notes |
|:--|:-----|:--------|:------|
| 1..N | `GET /api/?version=2.1&number={npi}` | one per NPI, `Promise.allSettled` | each yields 0/1 record; a rejected leg → `notFound[]`, not a thrown call |
| N+1 | merge | partition into `found[]` / `notFound[]`; throw `none_found` only if all miss | partial success is the norm |
