...
const EndpointSchema = z
  .object({
    ...
    use: z.string().optional().describe('Endpoint use, when present.'),
    contentType: z.string().optional().describe('Endpoint content type, when present.'),
  })
  .describe('A FHIR or Direct messaging endpoint.');
...
function renderRecord(r: z.infer<typeof FullProviderRecordSchema>): string {
  ...
  if (r.authorizedOfficial) {
    ...
    const name = [ao.firstName, ao.middleName, ao.lastName].filter(Boolean).join(' ');
    const bits = [
      name && `**Name:** ${name}`,
  ...
  if (r.otherNames.length) {
    ...
    for (const n of r.otherNames) {
      const nm = [n.organizationName, n.firstName, n.lastName, n.credential]
        .filter(Boolean)
        .join(' ');
...
export const getProviderTool = tool('npi_get_provider', {
  ...
  errors: [
    ...
    },
    {
      ...
      when: 'Every requested NPI returned no record.',
      recovery:
  ...
  output: z.object({
    ...
    notFound: z
      ...
      .describe(
        'NPIs that were well-formed but returned no record (deactivated or never enumerated).',
      ),
  }),
  ...
  async handler(input, ctx) {
    ...
    for (const [idx, res] of settled.entries()) {
      ...
      if (res.status === 'fulfilled') {
        ...
        notFound.push({ npi, reason: message });
      }
    }

    // All requested NPIs missed → throw none_found. Partial misses do not throw.
    if (found.length === 0) {
      throw ctx.fail(
        'none_found',
    ...
    if (notFound.length > 0) {
      ctx.enrich.notice(
        `${notFound.length} of ${unique.length} NPI(s) returned no record — they may be deactivated or never enumerated. Found ${found.length}.`,
      );
    }
