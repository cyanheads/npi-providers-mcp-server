...
Thanks for using `npi-providers-mcp-server`. Bugs, feature requests, and documentation gaps all belong in an issue — that's where they get read and picked up.
...
Pull requests are welcome; open an issue first for changes larger than a typo.

## Server bug or framework bug?

`npi-providers-mcp-server` is built on [@cyanheads/mcp-ts-core](https://github.com/cyanheads/mcp-ts-core), which handles transports, auth, config, logging, and telemetry. Sorting out which layer broke saves everyone a round-trip:
